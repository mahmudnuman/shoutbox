<?php

include_once "config/config.php";
class database{

        public $host = DB_HOST;
        public $user = DB_USER;
        public $pass = DB_PASS;
        public $dbname = DB_NAME;

        public $link;
        public $error;

    public function __construct()
    {
        $this->connectDB();
    }

    private function connectDB(){

            $this->link = new mysqli($this->host,$this->user,$this->pass,$this->dbname);
            if (!$this->link){

                $this->error="Connection fail".$this->link->connect_error;

            return false;
            }
        }

        public function select($query){

        $result= $this->link->query($query) or die($this->link->error.__LINE__);
        if ($result->num_rows > 0) {
return $result;
        }else {
            return false;
        }

        }
        public function insertDB($insert){


        $insert_row = $this->link->query($insert)or die($this->link->error.__LINE__);
            if ($insert_row){

        header("Location:index.php");
        exit();

            }
            else {

                die("Eror:(".$this->link->errorno.")".$this->link->error);
            }
        }
    public function updateDB($insert){


        $insert_row = $this->link->query($insert)or die($this->link->error.__LINE__);
        if ($insert_row){

            header("Location:index.php?msg=".urlencode('Data Updated Succesfully.'));
            exit();

        }
        else {

            die("Eror:(".$this->link->errorno.")".$this->link->error);
        }
    }
    public function deleteDATA($insert){


        $delete_row = $this->link->query($insert)or die($this->link->error.__LINE__);
        if ($delete_row){

            header("Location:index.php?msg=".urlencode('Data Deleted Succesfully.'));
            exit();

        }
        else {

            die("Eror:(".$this->link->errorno.")".$this->link->error);
        }
    }
}